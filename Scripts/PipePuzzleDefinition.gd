...
## Pipe puzzle: 3x3, source left of (0,0), sink below (2,2).
static func create_puzzle_3x3_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(3)
	puzzle.source_pos = Vector2i(-1, 0)
	...
	# Route: source(-1,0) → (0,0) → (1,0) → (2,0) ↓ (2,1) ↓ (2,2) → sink(2,3)
	puzzle.set_piece(0, 0, "straight", 1)  # left ↔ right
	puzzle.set_piece(1, 0, "straight", 1)  # left ↔ right
	puzzle.set_piece(2, 0, "corner",   2)  # left + down
	puzzle.set_piece(2, 1, "straight", 0)  # up ↕ down
	puzzle.set_piece(2, 2, "straight", 0)  # up ↕ down — exits to sink below

	return puzzle

## Pipe puzzle: 4x4, source above (0,0), sink right of (3,3).
static func create_puzzle_4x4_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(4)
	puzzle.source_pos = Vector2i(0, -1)
	puzzle.sink_pos = Vector2i(4, 3)

	for y in range(4):
		for x in range(4):
			puzzle.set_piece(x, y, "block", 0, true)

	# Route: source(0,-1) ↓ (0,0) → (1,0) → (2,0) → (3,0) ↓ (3,1) ↓ (3,2) ↓ (3,3) → sink(4,3)
	puzzle.set_piece(0, 0, "corner",   0)  # up + right
	puzzle.set_piece(1, 0, "straight", 1)  # left ↔ right
	puzzle.set_piece(2, 0, "straight", 1)  # left ↔ right
	puzzle.set_piece(3, 0, "corner",   2)  # left + down
	puzzle.set_piece(3, 1, "straight", 0)  # up ↕ down
	puzzle.set_piece(3, 2, "straight", 0)  # up ↕ down
	puzzle.set_piece(3, 3, "corner",   0)  # up + right — exits to sink right

	return puzzle

## Pipe puzzle: 5x4, source above (2,0), sink below (2,3). Route wraps left side.
static func create_puzzle_5x4_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(5, 4)
	puzzle.source_pos = Vector2i(2, -1)
	puzzle.sink_pos = Vector2i(2, 4)
	...
	# Route: source(2,-1) ↓ (2,0) ← (1,0) ← (0,0) ↓ (0,1) ↓ (0,2) ↓ (0,3) → (1,3) → (2,3) → sink(2,4)
	puzzle.set_piece(2, 0, "corner",   3)  # up + left
	puzzle.set_piece(1, 0, "straight", 1)  # left ↔ right
	puzzle.set_piece(0, 0, "corner",   1)  # right + down
	puzzle.set_piece(0, 1, "straight", 0)  # up ↕ down
	puzzle.set_piece(0, 2, "straight", 0)  # up ↕ down
	puzzle.set_piece(0, 3, "corner",   0)  # up + right
	puzzle.set_piece(1, 3, "straight", 1)  # left ↔ right
	puzzle.set_piece(2, 3, "corner",   2)  # left + down — exits to sink below

	return puzzle

## Pipe puzzle: 5x5, source left of (0,0), sink right of (4,4). Double-zigzag.
static func create_puzzle_5x5_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(5)
	puzzle.source_pos = Vector2i(-1, 0)
	puzzle.sink_pos = Vector2i(5, 4)

	for y in range(5):
		for x in range(5):
			puzzle.set_piece(x, y, "block", 0, true)

	# Route: source(-1,0) → (0,0) ↓...(0,4) → (1,4)→(2,4)→(3,4) ↑...(3,0) → (4,0) ↓...(4,4) → sink(5,4)
	puzzle.set_piece(0, 0, "corner",   2)  # left + down
	puzzle.set_piece(0, 1, "straight", 0)
	puzzle.set_piece(0, 2, "straight", 0)
	puzzle.set_piece(0, 3, "straight", 0)
	puzzle.set_piece(0, 4, "corner",   0)  # up + right
	puzzle.set_piece(1, 4, "straight", 1)
	puzzle.set_piece(2, 4, "straight", 1)
	puzzle.set_piece(3, 4, "corner",   3)  # up + left
	puzzle.set_piece(3, 3, "straight", 0)
	puzzle.set_piece(3, 2, "straight", 0)
	puzzle.set_piece(3, 1, "straight", 0)
	puzzle.set_piece(3, 0, "corner",   1)  # right + down
	puzzle.set_piece(4, 0, "corner",   2)  # left + down
	puzzle.set_piece(4, 1, "straight", 0)
	puzzle.set_piece(4, 2, "straight", 0)
	puzzle.set_piece(4, 3, "straight", 0)
	puzzle.set_piece(4, 4, "corner",   0)  # up + right — exits to sink right

	return puzzle

## Pipe puzzle: 6x5, source left of (0,0), sink right of (5,0). Deep U-shape.
static func create_puzzle_6x5_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(6, 5)
	puzzle.source_pos = Vector2i(-1, 0)
	puzzle.sink_pos = Vector2i(6, 0)

	for y in range(5):
		for x in range(6):
			puzzle.set_piece(x, y, "block", 0, true)

	# Route: source(-1,0) → (0,0) ↓...(0,4) → (1,4)→...(4,4) → (5,4) ↑...(5,0) → sink(6,0)
	puzzle.set_piece(0, 0, "corner",   2)  # left + down
	puzzle.set_piece(0, 1, "straight", 0)
	puzzle.set_piece(0, 2, "straight", 0)
	puzzle.set_piece(0, 3, "straight", 0)
	...
	puzzle.set_piece(1, 4, "straight", 1)
	puzzle.set_piece(2, 4, "straight", 1)
	puzzle.set_piece(3, 4, "straight", 1)
	puzzle.set_piece(4, 4, "straight", 1)
	puzzle.set_piece(5, 4, "corner",   3)  # up + left
	puzzle.set_piece(5, 3, "straight", 0)
	puzzle.set_piece(5, 2, "straight", 0)
	puzzle.set_piece(5, 1, "straight", 0)
	puzzle.set_piece(5, 0, "corner",   1)  # right + down — exits right to sink

	return puzzle

## Wire puzzle: 4x4, source above (1,-1), sink below (2,4).
static func create_wire_full_4x4_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(4, 4)
	puzzle.source_pos = Vector2i(1, -1)
	puzzle.sink_pos = Vector2i(2, 4)
	...
	# Route: source(1,-1) ↓ (1,0) → (2,0) ↓ (2,1) ↓ (2,2) ↓ (2,3) → sink(2,4)
	puzzle.set_piece(1, 0, "corner",   0)  # up + right
	puzzle.set_piece(2, 0, "corner",   2)  # left + down
	puzzle.set_piece(2, 1, "straight", 0)
	puzzle.set_piece(2, 2, "straight", 0)
	puzzle.set_piece(2, 3, "straight", 0)  # up ↕ down — exits to sink below

	return puzzle

## Wire puzzle: 5x4, source left of (-1,1), sink right of (5,2).
static func create_wire_full_5x4_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(5, 4)
	puzzle.source_pos = Vector2i(-1, 1)
	puzzle.sink_pos = Vector2i(5, 2)
	...
	# Route: source(-1,1) → (0,1) → (1,1) → (2,1) ↓ (2,2) → (3,2) → (4,2) → sink(5,2)
	puzzle.set_piece(0, 1, "straight", 1)  # left ↔ right
	puzzle.set_piece(1, 1, "straight", 1)
	puzzle.set_piece(2, 1, "corner",   2)  # left + down
	puzzle.set_piece(2, 2, "corner",   0)  # up + right
	puzzle.set_piece(3, 2, "straight", 1)
	puzzle.set_piece(4, 2, "straight", 1)  # exits right to sink

	return puzzle

## Wire puzzle: 5x5, source above (0,-1), sink right of (5,4). L-path down then across.
static func create_wire_full_5x5_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(5, 5)
	puzzle.source_pos = Vector2i(0, -1)
	puzzle.sink_pos = Vector2i(5, 4)
	...
	# Route: source(0,-1) ↓ (0,0)↓(0,1)↓(0,2)↓(0,3)↓(0,4) → (1,4)→(2,4)→(3,4)→(4,4) → sink(5,4)
	puzzle.set_piece(0, 0, "straight", 0)
	puzzle.set_piece(0, 1, "straight", 0)
	puzzle.set_piece(0, 2, "straight", 0)
	puzzle.set_piece(0, 3, "straight", 0)
	puzzle.set_piece(0, 4, "corner",   0)  # up + right
	...
	puzzle.set_piece(2, 4, "straight", 1)
	puzzle.set_piece(3, 4, "straight", 1)
	puzzle.set_piece(4, 4, "straight", 1)  # exits right to sink

	return puzzle

## Wire puzzle: 6x4, source above (3,-1), sink below (3,4). Hairpin around the grid.
static func create_wire_full_6x4_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(6, 4)
	puzzle.source_pos = Vector2i(3, -1)
	puzzle.sink_pos = Vector2i(3, 4)
	...
	# Route: source(3,-1) ↓ (3,0) ← (2,0) ← (1,0) ← (0,0) ↓ (0,1)↓(0,2)↓(0,3) → (1,3)→(2,3)→(3,3) → sink(3,4)
	puzzle.set_piece(3, 0, "corner",   3)  # up + left
	puzzle.set_piece(2, 0, "straight", 1)  # left ↔ right
	puzzle.set_piece(1, 0, "straight", 1)
	puzzle.set_piece(0, 0, "corner",   1)  # right + down
	puzzle.set_piece(0, 1, "straight", 0)
	puzzle.set_piece(0, 2, "straight", 0)
	puzzle.set_piece(0, 3, "corner",   0)  # up + right
	puzzle.set_piece(1, 3, "straight", 1)
	puzzle.set_piece(2, 3, "straight", 1)
	puzzle.set_piece(3, 3, "corner",   2)  # left + down — exits to sink below

	return puzzle

## Wire puzzle: 6x6, source above (0,-1), sink right of (6,3). Spiral-like path.
static func create_wire_full_6x6_ext_a() -> PipePuzzleDefinition:
	var puzzle := PipePuzzleDefinition.new(6, 6)
	puzzle.source_pos = Vector2i(0, -1)
	puzzle.sink_pos = Vector2i(6, 3)
	...
	# Route: source(0,-1) ↓ (0,0)↓...(0,4) → (1,4)→(2,4) → (3,4) ↑...(3,0) → (4,0)→(5,0) ↓ (5,1)↓(5,2)↓(5,3) → sink(6,3)
	puzzle.set_piece(0, 0, "straight", 0)
	puzzle.set_piece(0, 1, "straight", 0)
	puzzle.set_piece(0, 2, "straight", 0)
	puzzle.set_piece(0, 3, "straight", 0)
	...
	puzzle.set_piece(3, 1, "straight", 0)
	puzzle.set_piece(3, 0, "corner",   1)  # right + down
	puzzle.set_piece(4, 0, "straight", 1)
	puzzle.set_piece(5, 0, "corner",   2)  # left + down
	puzzle.set_piece(5, 1, "straight", 0)
	puzzle.set_piece(5, 2, "straight", 0)
	puzzle.set_piece(5, 3, "corner",   0)  # up + right — exits to sink right
